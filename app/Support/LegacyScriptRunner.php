...
class LegacyScriptRunner
{
    ...
    public static function run(Request $request, string $scriptPath, string $legacyUri, array $options = []): ?array
    {
        ...
        // Merge order: system env, root env, caller extras, forced flags, explicit DB_* overrides.
        // Use getenv() instead of $_ENV because $_ENV may be empty when
        // variables_order does not include 'E' (common on Windows).
        $systemEnv = getenv();
        $env = array_merge(is_array($systemEnv) ? $systemEnv : $_ENV, self::loadRootEnv(), $extraEnv, [
