  ...
  // Here uploadPlugin Code
    ...
    if(!is_uploaded_file($zip_file_tmp) || (!in_array($file_extension,$allowed,true) && !empty($zip_file))){
      ...
      if(move_uploaded_file($zip_file_tmp,"files/$zip_file")){
        ...
        $c_version = getStringBetween($readme,"Compatible gig-zone Version: ","\n");
        ...
        if(!file_exists("updator/plugin.sql") or !file_exists("updator/files.zip")){
          ...
          if(!is_dir("../plugins/$folderName")){
            ...
            if($c_version <= $current_version){
              ...
              echo "<script>alert_success('This Plugin Will Only Work On gig-zone $c_version And Above $c_version','index?plugins');</script>"; 
            }
