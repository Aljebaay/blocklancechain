    ...
    <div class="container">
        ...
        if (isset($_POST['insert_review'])) {
            ...
            if ($val->run() == false) {
                ...
                $avg = ( $total / ($ratings_count * 5) * 100); //## mehdi
                $update_proposal = $db->update("proposals", array("proposal_rating" => $avg), array("proposal_id" => $proposal_id));  //## mehdi
                $update_seller_rating = $db->update("sellers", array("seller_rating" => $avg), array("seller_id" => $proposal_seller_id));  //## mehdi
