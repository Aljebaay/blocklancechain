...
<?php
	...
	if (isset($_GET['delete_buyer_review'])) {
		...
		if ($delete_buyer_review) {
			...
			// $update_proposal = $db->update("proposals", ["proposal_rating" => $updated_propoasl_rating], ["proposal_id" => $proposal_id]);  //## mehdi
			$update_proposal = $db->update("proposals", ["proposal_rating" => $avg], ["proposal_id" => $proposal_id]);  //## mehdi
			$update_seller_rating = $db->query("update sellers set seller_rating=$avg where seller_id='$seller_id'");
