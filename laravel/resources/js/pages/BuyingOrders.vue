...
async function fetchOrders(page = 1) {
    ...
    error.value = null;
    try {
        const { data } = await axios.get(`/api/v1/orders/buying/${currentStatus.value}`, { params: { page } });
...
<template>
    <div class="container mx-auto px-4 py-8">
        <h1 class="text-2xl font-bold text-gray-900 mb-8">Buying Orders</h1>

        <div class="flex gap-2 mb-6 overflow-x-auto pb-2">
            <router-link
                v-for="tab in statusTabs"
                :key="tab.value"
                :to="`/orders/buying/${tab.value}`"
                :class="[
                ...
                >
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-gray-500">Seller: {{ order.seller?.seller_user_name || '-' }}</p>
                        </div>
