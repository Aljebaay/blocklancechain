...
@section('title', ($site_name ?? 'GigZone') . ' - Search Results For ' . session('search_query', ''))

@section('meta_description'){{ $site_desc ?? '' }}@endsection

@section('body_class', 'bg-white is-responsive')
...
<div class="container-fluid mt-5"> <!-- Container start -->
  <div class="row mt-3">
    <div class="col-md-12">
      <center>
        <h1>{{ $lang['search']['title'] ?? 'Search Results' }}</h1>
        <p class="lead pb-5"> "{{ $search_query }}" </p>
      </center>
      <hr>
    </div>
  </div>
  <div class="row mt-3">
    <div class="col-lg-3 col-md-4 col-sm-12 {{ $langDir == 'right' ? 'order-2 order-sm-1' : '' }}">
      @include('legacy.partials.search-sidebar')
    </div>
    <div class="col-lg-9 col-md-8 col-sm-12 {{ $langDir == 'right' ? 'order-1 order-sm-2' : '' }}">
      <div class="row flex-wrap proposals" id="search_proposals">
        @include('legacy.partials.search-results')
      </div>
      <div id="wait"></div>
      <br>
...
$(document).ready(function(){
    ...
    if($(".get_seller_country:checked").length > 0){
      ...
      $.each(aInputs,function(key,oInput){
        if(oInput.checked){
          ...
          if(city.length){ cities[iKey] = city_name; }
          iKey++;
        };
      });
      if(cities.length > 0){ $(".seller-cities").removeClass('d-none'); }
      else{ $(".seller-cities").addClass('d-none'); }
    }else{
  ...
  $(".get_seller_city").click(function(){ if($(".get_seller_city:checked").length > 0 ) { $(".clear_seller_city").show(); }else{ $(".clear_seller_city").hide(); } });
  $(".get_cat_id").click(function(){ if($(".get_cat_id:checked").length > 0 ) { $(".clear_cat_id").show(); } else{ $(".clear_cat_id").hide(); } });
  $(".get_delivery_time").click(function(){ if($(".get_delivery_time:checked").length > 0 ) { $(".clear_delivery_time").show(); } else{ $(".clear_delivery_time").hide(); } });
  $(".get_seller_level").click(function(){ if($(".get_seller_level:checked").length > 0 ) { $(".clear_seller_level").show(); } else{ $(".clear_seller_level").hide(); } });
  $(".get_seller_language").click(function(){ if($(".get_seller_language:checked").length > 0 ) { $(".clear_seller_language").show(); } else{ $(".clear_seller_language").hide(); } });
...
function clearCountry(){ $('.get_seller_country').prop('checked',false); $('.get_seller_city').prop('checked',false); $(".seller-cities").addClass('d-none'); get_search_proposals(); }
function clearCity(){ $('.get_seller_city').prop('checked',false); get_search_proposals(); }
function clearCat(){ $('.get_cat_id').prop('checked',false); get_search_proposals(); }
function clearDelivery(){ $('.get_delivery_time').prop('checked',false); get_search_proposals(); }
function clearLevel(){ $('.get_seller_level').prop('checked',false); get_search_proposals(); }
function clearLanguage(){ $('.get_seller_language').prop('checked',false); get_search_proposals(); }
</script>
@endsection
