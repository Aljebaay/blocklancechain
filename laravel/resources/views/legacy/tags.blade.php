...
@section('title'){{ $site_name }} - {{ ucfirst($tag ?? '') }}@endsection
...
<div class="container-fluid mt-5"> <!-- Container start -->
	<div class="row">
		<div class="col-md-12">
			<center>
			<h1> {{ ucfirst($tag ?? '') }} </h1>
			<p class="lead">{{ str_replace('{tag}', $tag ?? '', $lang['tag_proposals']['desc'] ?? 'Proposals tagged with {tag}') }}</p>
			</center>
			<hr class="mt-5 pt-2">
		</div>
	</div>
