...
$legacyPassthrough = static function (string $path): void {
    ...
    $path = $path === '' ? '/' : $path;
    $publicRoot = realpath(base_path('..' . DIRECTORY_SEPARATOR . 'public'));
    $platformRoot = realpath(
        base_path('..' . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'Modules' . DIRECTORY_SEPARATOR . 'Platform')
    );
    $mimeMap = [
    ...
    $serveStaticFile = static function (string $filePath, string $extension, array $mimeMap): void {
        if (!headers_sent()) {
            $mime = $mimeMap[$extension] ?? (@mime_content_type($filePath) ?: 'application/octet-stream');
            if ($mime !== '') {
    ...
    $resolveStaticCandidate = static function ($basePath, string $requestPath): ?string {
    ...
    // Fast path for static assets when serving through `php artisan serve`.
    if ($path !== '/') {
        $requestPathOnly = parse_url($path, PHP_URL_PATH) ?: $path;
        $extension = strtolower(pathinfo($requestPathOnly, PATHINFO_EXTENSION));
        if ($extension !== '' && isset($mimeMap[$extension])) {
            $directPublicFile = $resolveStaticCandidate($publicRoot, $requestPathOnly);
            if (is_string($directPublicFile)) {
                $serveStaticFile($directPublicFile, $extension, $mimeMap);
            }

            $platformFile = $resolveStaticCandidate($platformRoot, $requestPathOnly);
            if (is_string($platformFile)) {
                $serveStaticFile($platformFile, $extension, $mimeMap);
            }
        }
    }

    $isRedirectStatus = static fn (int $status): bool => in_array($status, [301, 302, 303, 307, 308], true);
    $hasHeaderLine = static function (array $headerLines, string $name): bool {
    ...
    $isUsableResult = static function (int $status, string $body, array $headerLines) use ($isRedirectStatus, $hasHeaderLine): bool {
        if ($status <= 0) {
            return false;
        }
        ...
        return $isRedirectStatus($status) && ($headerLines === [] || $hasHeaderLine($headerLines, 'Location'));
    };
    $applyHeaderLines = static function (array $headerLines): bool {
        ...
        foreach ($headerLines as $headerLine) {
            ...
            if (strcasecmp($headerName, 'Content-Length') === 0 || strcasecmp($headerName, 'Transfer-Encoding') === 0) {
                continue;
            }
            ...
            $replace = strcasecmp($headerName, 'Set-Cookie') !== 0;
    ...
    if (!$isUsableResult($status, $body, $headerLines) && $path !== '/') {
            ...
            if ($direct !== false && is_file($direct)) {
                ...
                if ($ext === 'php') {
                    ...
                    if (is_array($directResult)) {
                        ...
                        $headerLines = is_array($directResult['headers'] ?? null) ? $directResult['headers'] : [];
                    }
                } else {
    ...
    if (!$isUsableResult($status, $body, $headerLines)) {
        abort(500);
    }

    if (!headers_sent()) {
        http_response_code($status);
        $hasContentType = $applyHeaderLines($headerLines);
...
Route::prefix('/_app')->group(function () {
    ...
    Route::match(['get', 'post'], '/migrate/orders/cancel_payment.php', CancelPaymentController::class);
