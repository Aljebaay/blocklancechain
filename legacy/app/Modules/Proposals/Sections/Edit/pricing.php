...
if (!isset($row_proposal) || !is_object($row_proposal)) {
  return;
}

if (!isset($proposal_id)) {
  $proposal_id = isset($row_proposal->proposal_id) ? (int) $row_proposal->proposal_id : 0;
}

if (!isset($d_proposal_price)) {
  $d_proposal_price = (string) $row_proposal->proposal_price;
}
if (!isset($d_proposal_revisions)) {
  $d_proposal_revisions = (string) $row_proposal->proposal_revisions;
}
if (!isset($d_proposal_status)) {
  $d_proposal_status = (string) $row_proposal->proposal_status;
}
if (!isset($d_delivery_id)) {
  $d_delivery_id = (string) $row_proposal->delivery_id;
}

if (!isset($min_proposal_price)) {
  $get_payment_settings = $db->select("payment_settings");
  $row_payment_settings = $get_payment_settings->fetch();
  $min_proposal_price = $row_payment_settings ? $row_payment_settings->min_proposal_price : 0;
}

if (!isset($row_general_settings) || !is_object($row_general_settings)) {
  return;
}
...
<div class="packages"><?php include $platformRoot . "/proposals/sections/edit/packages.php"; ?></div>
...
<div class="card rounded-0">
  ...
  <a data-toggle="collapse" href="#insert-extra" class="small text-success">+ Add Extra</a>
   <div class="tabs accordion mt-2" id="allTabs"><!--- All Tabs Starts --->
      <?php include $platformRoot . "/proposals/sections/edit/extras.php"; ?>
    </div><!--- All Tabs Ends --->
  </div>
</div>
