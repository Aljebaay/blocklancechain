...
$checks = [
    ['id' => 'home-page', 'path' => '/', 'expectedStatuses' => [200, 302], 'bodyContainsAny' => ['<html', 'install.php', 'login', '<title'], 'snapshot' => 'home', 'dbDependent' => true, 'expectHandler' => 'legacy'],
    ['id' => 'login-page', 'path' => '/login', 'expectedStatuses' => [200, 302], 'bodyContainsAny' => ['login', '<form', '<title', 'install.php'], 'snapshot' => 'login', 'dbDependent' => true, 'expectHandler' => 'legacy'],
    ['id' => 'admin-login', 'path' => '/admin/login.php', 'expectedStatuses' => [200, 302], 'bodyContainsAny' => ['ADMIN', 'admin', '<form', '<title'], 'snapshot' => 'admin-login', 'dbDependent' => true, 'expectHandler' => 'legacy'],
    ['id' => 'index-alias', 'path' => '/index', 'expectedStatuses' => [200, 302], 'bodyContainsAny' => ['<html', 'install.php', 'login'], 'dbDependent' => true, 'expectHandler' => 'legacy'],
    ...
    ['id' => 'laravel-health', 'path' => '/_app/health', 'expectedStatuses' => [200], 'bodyContainsAny' => ['"status":"ok"', '"status": "ok"', 'status":"ok"'], 'expectHandler' => 'laravel'],
    ['id' => 'laravel-system-info', 'path' => '/_app/system/info', 'expectedStatuses' => [200], 'bodyContainsAny' => ['"status":"ok"', '"status": "ok"', 'status":"ok"'], 'expectHandler' => 'laravel'],
    ['id' => 'laravel-migrate-fetch-subcategory', 'method' => 'POST', 'path' => '/_app/migrate/requests/fetch_subcategory', 'headers' => ['Content-Type: application/x-www-form-urlencoded'], 'body' => 'category_id=1', 'expectedStatuses' => [200], 'bodyContainsAny' => ['<option', "window.open('../login"], 'expectHandler' => 'laravel'],
    ['id' => 'laravel-migrate-proposal-pricing', 'method' => 'POST', 'path' => '/_app/migrate/proposals/ajax/check/pricing', 'headers' => ['Content-Type: application/x-www-form-urlencoded'], 'body' => 'proposal_id=1&proposal_price=5&proposal_revisions=1&delivery_id=1', 'expectedStatuses' => [200], 'bodyContainsAny' => ['true', 'false', "window.open('../login", 'install.php'], 'expectHandler' => 'laravel'],
    ['id' => 'laravel-migrate-proposal-pricing-alias', 'method' => 'POST', 'path' => '/_app/migrate/proposal/pricing_check', 'headers' => ['Content-Type: application/x-www-form-urlencoded'], 'body' => 'proposal_id=1&proposal_price=5&proposal_revisions=1&delivery_id=1', 'expectedStatuses' => [200], 'bodyContainsAny' => ['true', 'false', "window.open('../login", 'install.php'], 'expectHandler' => 'laravel'],
    ['id' => 'laravel-migrate-apis-index', 'path' => '/_app/migrate/apis/index.php?/apis/register', 'expectedStatuses' => [200], 'bodyContainsAny' => ['CodeIgniter', '<html', '<title', 'ERROR: Not Found'], 'dbDependent' => true, 'expectHandler' => 'laravel'],
    ['id' => 'laravel-migrate-pause-request', 'path' => '/_app/migrate/requests/pause_request?request_id=0', 'expectedStatuses' => [200], 'bodyContainsAny' => ["window.open('manage_requests'", "window.open('../login", 'manage_requests', 'login'], 'dbDependent' => true, 'expectHandler' => 'laravel'],
    ['id' => 'laravel-migrate-active-request', 'path' => '/_app/migrate/requests/active_request', 'expectedStatuses' => [200], 'bodyContainsAny' => ['request', "window.open('../login", '<html', '<title'], 'dbDependent' => true, 'expectHandler' => 'laravel'],
    ['id' => 'laravel-migrate-manage-request', 'path' => '/_app/migrate/requests/manage_requests', 'expectedStatuses' => [200], 'bodyContainsAny' => ['manage_requests', "window.open('../login", '<html', '<title'], 'dbDependent' => true, 'expectHandler' => 'laravel'],
    ['id' => 'laravel-migrate-resume-request', 'path' => '/_app/migrate/requests/resume_request?request_id=0', 'expectedStatuses' => [200], 'bodyContainsAny' => ["window.open('manage_requests'", "window.open('../login", 'manage_requests', 'login'], 'dbDependent' => true, 'expectHandler' => 'laravel'],
    ['id' => 'laravel-migrate-create-request', 'method' => 'POST', 'path' => '/_app/migrate/requests/create_request', 'headers' => ['Content-Type: application/x-www-form-urlencoded'], 'body' => 'request_title=', 'expectedStatuses' => [200], 'bodyContainsAny' => ["window.open('../login", 'manage_requests', 'request'], 'dbDependent' => true, 'writeDependent' => true, 'expectHandler' => 'laravel'],
    ['id' => 'laravel-migrate-update-request', 'method' => 'POST', 'path' => '/_app/migrate/requests/update_request', 'headers' => ['Content-Type: application/x-www-form-urlencoded'], 'body' => 'request_id=0', 'expectedStatuses' => [200], 'bodyContainsAny' => ["window.open('../login", 'manage_requests', 'request'], 'dbDependent' => true, 'writeDependent' => true, 'expectHandler' => 'laravel'],
    ...
    ['id' => 'requests-module-laravel', 'path' => '/requests/manage_requests', 'expectedStatuses' => [200, 302], 'bodyContainsAny' => ['manage_requests', '<html', '<title', "window.open('../login"], 'dbDependent' => true, 'laravelOnly' => true],
    ...
    // X-Handler parity probes — verify headers in both modes
    ['id' => 'handler-header-legacy-home', 'path' => '/', 'expectedStatuses' => [200, 302], 'expectHandler' => 'auto', 'dbDependent' => true],
    ['id' => 'handler-header-laravel-health', 'path' => '/_app/health', 'expectedStatuses' => [200], 'expectHandler' => 'laravel', 'bodyContainsAny' => ['status']],
];
...
function evaluateResponse(array $check, array $response): array
{
    ...
    // X-Handler parity assertion
    if (isset($check['expectHandler']) && is_string($check['expectHandler']) && $check['expectHandler'] !== '') {
        $xHandler = isset($headers['x-handler']) && is_string($headers['x-handler']) ? $headers['x-handler'] : '';
        $expected = $check['expectHandler'];

        if ($expected === 'auto') {
            // Just verify the header exists (any value)
            if ($xHandler === '') {
                $errors[] = 'Missing X-Handler header (expected either legacy or laravel).';
            }
        } elseif ($xHandler === '') {
            $errors[] = "Missing X-Handler header (expected '{$expected}').";
        } elseif ($xHandler !== $expected) {
            $errors[] = "X-Handler header mismatch: expected '{$expected}', got '{$xHandler}'.";
        }
    }
