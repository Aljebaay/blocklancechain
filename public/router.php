...
/**
 * Set X-Handler: legacy header for parity verification.
 * Safe: headers are not sent to the client until output begins.
 */
function blc_set_legacy_handler(): void
{
    if (!headers_sent()) {
        header('X-Handler: legacy');
    }
}

/**
 * Clean up headers/status after a failed Laravel attempt before legacy fallback.
 * Prevents header leakage from partial Laravel execution.
 */
function blc_prepare_legacy_fallback(): void
{
    if (!headers_sent()) {
        header_remove();
        http_response_code(200);
    }
    blc_set_legacy_handler();
}
